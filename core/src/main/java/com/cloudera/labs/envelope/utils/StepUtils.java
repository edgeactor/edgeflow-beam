...
public class StepUtils {
    ...
    public static boolean hasStructuredStreamingStep(Set<Step> steps) {
        for (Step step : steps) {
            if (step instanceof StructuredStreamingStep) {
                return true;
            }
        }
    ...
    public static Set<StructuredStreamingStep> getStructuredStreamingSteps(Set<Step> steps) {
        Set<StructuredStreamingStep> steamingSteps = Sets.newHashSet();

        for (Step step : steps) {
            if (step instanceof StructuredStreamingStep) {
                steamingSteps.add((StructuredStreamingStep) step);
            }
        }
    ...
    public static Set<Step> getIndependentNonStructuredStreamingSteps(Set<Step> steps) {
        ...
        for (String stepName : stepNames) {
            ...
            if (!stepConfig.hasPath(Runner.TYPE_PROPERTY) ||
                ...
                if (stepConfig.hasPath(DataStep.INPUT_TYPE)) {
                    ...
                    } else if (stepInput instanceof StructuredStreamingInput) {
                        LOG.debug("Adding structured streaming step: " + stepName);
                        step = new StructuredStreamingStep(stepName);
                    } else {
